...
from app.utils import createWeekDays, addPlaces, addDomesticFlights, addInternationalFlights
...
def register_view(request):
    if request.method == "POST":
        ...
        try:
            user = SimpleUser.objects.create_user(username, email, password)
    ...
    if request.method == 'POST':
        if request.user.is_authenticated:
            ...
            for i in range(1,int(passengerscount)+1):
                ...
                passengers.append(Passenger.objects.create(first_name=fname,last_name=lname,gender=gender.lower()).save())
...
def get_ticket(request):
    ...
    pdf = render_to_pdf('tickets.html', data)
